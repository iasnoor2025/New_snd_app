...
export default function Edit({ auth, rentalId, rentalItem, equipment, operators }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        equipment_id: rentalItem.equipment_id.toString(),
        operator_id: rentalItem.operator_id?.toString() || '',
        rate: rentalItem.rate.toString(),
        rate_type: rentalItem.rate_type,
        days: rentalItem.days.toString(),
        discount_percentage: rentalItem.discount_percentage.toString(),
        notes: rentalItem.notes || '',
    })

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('rentals.items.update', { rental: rentalId, item: rentalItem.id }));
    };

    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Edit Rental Item</h2>}
