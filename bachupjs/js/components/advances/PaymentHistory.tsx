...
export function PaymentHistory({
  ...
  const handleDeletePayment = (paymentId: number) => {
    ...
    router.delete(route('advances.payment-history.delete', {
      employee: employeeId,
      payment: paymentId
    }), {
      onSuccess: () => {
        ...
        <div className="flex items-center gap-2">
          ...
          {showOnlyLast && (
            <Button variant="outline" size="sm" asChild>
              <Link href={route('advances.payment-history', { employee: employeeId })}>
                View Full History
              </Link>
            </Button>
          )}
      ...
      <div className="space-y-8">
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button
                          ...
                          className="h-8 w-8"
                          onClick={() => {
                            const url = route('advances.payment.receipt', {
                              employee: employeeId,
                              payment: payment.id
                            })
        ...
        {!showOnlyLast && (
            ...
            <div className="text-center text-sm text-muted-foreground mt-2">
              Page {currentPage} of {totalPages} • Showing {monthlyHistory.length} months
            </div>
          </>
        )}
      </div>
            ...
            <Button
              ...
              onClick={() => {
                if (selectedPayment) {
                  ...
                  ToastService.error("No payment selected for deletion");
                }
              }}
              Delete
            </Button>
