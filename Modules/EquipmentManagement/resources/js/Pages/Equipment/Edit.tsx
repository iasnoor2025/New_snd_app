    ...
    router.put(window.route('equipment.update', { equipment: equipment.id }), formattedValues, {
      ...
      onError: (errors) => {
        ...
        if (errors.door_number) {
          ToastService.error(`Door number "${values.door_number}" is already in use. Please choose a different door number.`);
          ...
          ToastService.error(`Name error: ${errors.name}`);
        } else if (errors.model) {
          ToastService.error(`Model error: ${errors.model}`);
        } else if (errors.serial_number) {
          ToastService.error(`Serial number error: ${errors.serial_number}`);
        } else if (errors.status) {
          ToastService.error(`Status error: ${errors.status}`);
        } else if (errors.location_id) {
          ToastService.error(`Location error: ${errors.location_id}`);
        } else if (errors.category_id) {
          ToastService.error(`Category error: ${errors.category_id}`);
        } else {
          ToastService.error("Failed to update equipment. Please check the form for errors.");
        }
      },
      preserveScroll: true,
    });
